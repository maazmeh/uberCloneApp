...
const FetchDestinationModal = ({ isVisible, onClose, onSelectLocation }) => {
  const mapViewRef = useRef(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [locations, setLocations] = useState([]);
  const [selectedLocation, setSelectedLocation] = useState(null);
  const [markerPositionLat, setMarkerPositionLat] = useState<any>(9.0765);
  const [markerPositionLong, setMarkerPositionLong] = useState<any>(7.3986);
...
export default FetchDestinationModal;